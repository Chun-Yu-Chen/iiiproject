# -*- coding: utf-8 -*- 
import requests
import urlparse
from bs4 import BeautifulSoup,Comment
def remove_space(element):
	return  "".join(element.strip().split(" "))
##取得頁數
file = open('tainan_geography_spot.txt','w')
file.write("景點名稱|旅遊區域|旅遊主題|相關網址|地址|電話|美食資訊|住宿資訊|附近景點|建議停留|景點介紹"+"\n")
tainan_geography_link='http://tour.tainan.gov.tw/view.aspx?subject=geography'
tainan_geography = requests.get(tainan_geography_link) 
tainan_geography_text = tainan_geography.text.encode('utf-8')
tainan_geography_soup=BeautifulSoup(tainan_geography_text)
tainan_geography_page_number = tainan_geography_soup.find('span',{"id":"View1_SpotList1_lblPageCount"}).text.encode('utf-8')

##每頁取資料
for page in range(1,int(tainan_geography_page_number)+1):
    form_data_number='第'+str(page)+'頁'

    payload = { '__EVENTTARGET':'View1:SpotList1:ddlPageList'
      ,'__EVENTARGUMENT':''
      ,'__VIEWSTATE':''
      ,'View1:SpotList1:SpotQuery1:ddlSubject':'geography'
      ,'View1:SpotList1:SpotQuery1:ddlTown':''
      ,'View1:SpotList1:ddlPageList':form_data_number}
	  
    tainan_geography = requests.post(tainan_geography_link, data=payload) 
    tainan_geography_text = tainan_geography.text.encode('utf-8')
    tainan_geography_soup=BeautifulSoup(tainan_geography_text)
    tainan_geography_table=tainan_geography_soup.find('table',{"id":"View1_SpotList1_dtlList"})
    tainan_geography_spans = tainan_geography_table.findAll('span',{"id":"main-wrapper"})
	##另開連結取得景點名稱
    for tainan_geography_span in tainan_geography_spans:
        geography_alink = tainan_geography_span.find('a',{"href":True})['href']
        geography_link=urlparse.urljoin(tainan_geography_link,geography_alink)
        geography_page=requests.get(geography_link)
        geography_page_text = geography_page.text.encode('utf-8')
        geography_page_soup=BeautifulSoup(geography_page_text)
        
        comments = geography_page_soup.findAll(text=lambda text:isinstance(text, Comment))
        [comment.extract() for comment in comments]
        
        geography_name = remove_space(geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblName"}).text.encode('utf-8'))
        geography_region = remove_space(geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblTownName"}).text.encode('utf-8'))

        geography_type = remove_space(geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblSubjectName"}).text.encode('utf-8'))

        geography_content = remove_space(geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblContent"}).text.encode('utf-8'))

        geography_url_a = geography_page_soup.find('a',{"id":"View1_SpotDetail1_aUrl"})
        if geography_url_a is not None:
            geography_url = remove_space(geography_url_a['href'].encode('utf-8'))
        else:
            geography_url="暫無提供"
        
        geography_address_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblAddress"})
        if geography_address_span is not None:
            geography_address = remove_space(geography_address_span.text.encode('utf-8'))
        else:
            geography_address = "暫無提供"
        
        geography_phone_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblTel"})
        if geography_phone_span is not None:
            geography_phone = remove_space(geography_phone_span.text.encode('utf-8'))
        else:
            geography_phone = "暫無提供"
      
        geography_food_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_dtlSpotToFineFood"})
        if geography_food_span is not None:
            geography_food=""
            geography_food_spans = geography_food_span.findAll('span',{'id':True})
            count=len(geography_food_spans)
            for food_span in geography_food_spans:
                count-=1
                geography_food +=remove_space(food_span.text.encode('utf-8'))
                if count!=0:
                    geography_food +="/"
        else:
            geography_food = "暫無提供"
        
        
        geography_house_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_dtlSpotToHospice"})
        if geography_house_span is not None:
            geography_house=""
            geography_house_spans = geography_house_span.findAll('span',{'id':True})
            count=len(geography_house_spans)
            for house_span in geography_house_spans:
                count-=1
                geography_house +=remove_space(house_span.text.encode('utf-8'))
                if count!=0:
                    geography_house +="/"
        else:
            geography_house = "暫無提供"

        
        geography_spot_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_dtlSpotToSpot"})
        if geography_spot_span is not None:
            geography_spot=""
            geography_spot_as = geography_spot_span.findAll('a',{'id':True})
            count=len(geography_spot_as)
            for spot_a in geography_spot_as:
                count-=1
                geography_spot +=remove_space(spot_a.text.encode('utf-8'))
                if count!=0:
                    geography_spot +="/"
        else:
            geography_spot = "暫無提供"
       
        
        geography_proposal_span=geography_page_soup.find('span',{"id":"View1_SpotDetail1_lblProposal"})
        if geography_proposal_span is not None:
            geography_proposal = remove_space(geography_proposal_span.text.encode('utf-8'))
        else:
            geography_proposal="暫無提供"
        file.write(geography_name+"|"+geography_region+"|"+geography_type+"|"+geography_url+"|"+geography_address+"|")
        file.write(geography_phone+"|"+geography_food+"|"+geography_house+"|"+geography_spot+"|"+geography_proposal+"|"+geography_content+"\n")
file.close()